...
public class BookController {
    ...
    private fit.hutech.spring.entities.User getCurrentUser(Authentication authentication) {
        ...
        if (principal instanceof org.springframework.security.core.userdetails.UserDetails) {
            return userService.findByUsername(((org.springframework.security.core.userdetails.UserDetails) principal)
    ...
    public String showAllBooks(@NotNull Model model,
        ...
        if (currentUser != null) {
            ...
            model.addAttribute("isAdmin", currentUser.getRoles().stream().anyMatch(r -> r.getName().equals("ADMIN")));
        }
    ...
    public String editBookForm(@NotNull Model model, @PathVariable long id, Authentication authentication) {
        ...
        if (!canEditOrDelete(book, currentUser)) {
            return "redirect:/books?error=Báº¡n khÃ´ng cÃ³ quyá»n chá»‰nh sá»­a sÃ¡ch nÃ y";
        }
    ...
    public String deleteBook(@PathVariable long id, Authentication authentication) {
        ...
        if (!canEditOrDelete(bookOpt.get(), currentUser)) {
            return "redirect:/books?error=Báº¡n khÃ´ng cÃ³ quyá»n xÃ³a sÃ¡ch nÃ y";
        }
    ...
    public String addToCart(HttpServletRequest request,
        ...
        try {
            ...
            redirectAttributes.addFlashAttribute("successMessage",
                    "ÄÃ£ thÃªm \"" + book.getTitle() + "\" vÃ o giá» hÃ ng.");
    ...
    public ResponseEntity<byte[]> exportBooks() throws Exception {
        byte[] bytes = excelService.exportBooks();
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=books-export.xlsx")
                .contentType(
    ...
    public String importBooks(@RequestParam("file") org.springframework.web.multipart.MultipartFile file,
            ...
            RedirectAttributes redirectAttributes) {
        if (file.isEmpty()) {
            redirectAttributes.addFlashAttribute("errorMessage", "Vui lÃ²ng chá»n file Excel Ä‘á»ƒ import");
            return "redirect:/books";
        }
        try {
            var currentUser = getCurrentUser(authentication);
